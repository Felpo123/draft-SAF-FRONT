...
export default function MapComponent({ initialIncidentId }) { // Recibe el ID inicial como prop
  ...
  const [selectedIncidente, setSelectedIncidente] = useState(initialIncidentId || ''); // Almacena el incidente seleccionado
  const [incidentes, setIncidentes] = useState([]); // Almacena los IDs de los incidentes

  // Función para extraer fechas e IDs
  const extractDatesAndIds = (geojsonData) => {
    const fechasUnicas = [];
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const wfsUrl = 'geoserver/desafio/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=desafio:incidentes&outputFormat=application/json';
      try {
            ...
            const transformedCoordinates = geometry.coordinates.map((coords) => {
              return coords.map((coordArray) => {
                return coordArray.map((coord) => {
                  const transformedCoord = proj4('EPSG:3857', 'EPSG:4326', [coord[0], coord[1]]);
  ...
  const handleIncidenteChange = (event) => {
    ...
    const filteredData = {
      ...
      features: geojsonDataRef.current.features.filter((feature) => feature.properties.id_evento === newId),
    };
  ...
  const handleDateChange = (newIndex) => {
    ...
    const filteredData = {
      ...geojsonDataRef.current,
      features: geojsonDataRef.current.features.filter((feature) =>
